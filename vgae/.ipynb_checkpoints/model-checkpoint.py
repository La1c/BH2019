...
from vgae import File_Reader
from vgae import utils
...
class VGAE(object):
        ...
        def decode(self):
            ...
            matrix_pred = tf.nn.sigmoid(tf.matmul(z, z, transpose_a=False, transpose_b=True))

            return matrix_pred
        
#         def transform(self, adjacency):
#             sp_adjacency = File_Reader.dense_to_sparse(adjacency)
#             norm_adjacency = File_Reader.normalize_adjacency(adjacency)
#             feed_dict = {self.adjacency:sp_adjacency[0:2], self.norm_adj_mat:norm_adj_mat[0:2], self.keep_prob:args['keep_prob']}
        ...
        def forward(self, adjacency):
            sp_adjacency = File_Reader.dense_to_sparse(adjacency)
            #norm_adj_mat = File_Reader.normalize_adjacency(adjacency)
            feed_dict = {self.adjacency:sp_adjacency[0:2],
                         self.norm_adj_mat:norm_adj_mat[0:2],
                         self.keep_prob:self.keep_prob_param
                        }
            self.mu_np, self.sigma_np = self.sess.run([self.mu, self.sigma], feed_dict=feed_dict)
            return self.predict()
            
      

        def train(self, args, adjacency_list):
            self.keep_prob_param = args['keep_prob']
            for i in range(self.max_iteration):
                ...
                #if i%10 == 0:
                print("At step {0} \n Loss: {1} \n Average Precision: {2}.".format(i,
                                                                                       np.mean(epoch_losses),
                                                                                       np.mean(epoch_ap)))
                
                print('Epoch time {}'.format(time() - ep_start))
